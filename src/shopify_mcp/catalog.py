...
BULK_IMPORT_KIND = {"type": "string", "enum": ["PRODUCT_CREATE", "PRODUCT_UPDATE", "METAFIELDS_SET", "METAOBJECT_CREATE", "METAOBJECT_UPDATE"]}
...
DEFINITIONS = [
    ...
    ToolDefinition(_tool("shopify_prepare_bulk_export", "Gera internamente uma exportação JSONL de formato conhecido; não aceita GraphQL arbitrário.", BULK_EXPORT_FIELDS, ["resource"]), "prepare_bulk_export", "bulk_operations", False, frozenset({"full"})),
    ToolDefinition(_tool("shopify_start_bulk_export", "ESCRITA: inicia a exportação Bulk previamente preparada e retorna um job para acompanhamento.", {**BULK_EXPORT_FIELDS, "confirmationToken": {"type": "string", "minLength": 32, "maxLength": 256}}, ["resource", "confirmationToken"], write=True), "start_bulk_export", "bulk_operations", True, frozenset({"full"})),
    ...
    ToolDefinition(_tool("shopify_prepare_bulk_import_staged_upload", "Prepara um alvo POST exclusivo para JSONL de variáveis Bulk; o MCP não lê nem envia o arquivo.", BULK_IMPORT_STAGE_FIELDS, ["filename", "fileSize"]), "prepare_bulk_import_staged_upload", "bulk_operations", False, frozenset({"full"})),
    ToolDefinition(_tool("shopify_create_bulk_import_staged_upload", "ESCRITA: gera credenciais temporárias para upload direto do JSONL à Shopify.", {**BULK_IMPORT_STAGE_FIELDS, "confirmationToken": {"type": "string", "minLength": 32, "maxLength": 256}}, ["filename", "fileSize", "confirmationToken"], write=True), "create_bulk_import_staged_upload", "bulk_operations", True, frozenset({"full"})),
    ToolDefinition(_tool("shopify_prepare_bulk_import", "Prepara importação por mutation interna conhecida, vinculada ao path, hash e quantidade de linhas declarados.", BULK_IMPORT_FIELDS, ["kind", "stagedUploadPath", "clientIdentifier", "lineCount", "sha256", "acknowledgeUnorderedExecution"]), "prepare_bulk_import", "bulk_operations", False, frozenset({"full"})),
    ToolDefinition(_tool("shopify_start_bulk_import", "ESCRITA: inicia importação Bulk previamente preparada; cada linha JSONL é executada independentemente e fora de ordem.", {**BULK_IMPORT_FIELDS, "confirmationToken": {"type": "string", "minLength": 32, "maxLength": 256}}, ["kind", "stagedUploadPath", "clientIdentifier", "lineCount", "sha256", "acknowledgeUnorderedExecution", "confirmationToken"], write=True), "start_bulk_import", "bulk_operations", True, frozenset({"full"})),
